...
#                  ▼                             ▼
#      ┌─────────────────────┐         ┌─────────────────────┐
#      │  RDI Feature Branch │         │  RAI Feature Branch │
#      │  (CT Modules, etc.) │         │ ((2+1)D Conv, etc.) │
#      └────────┬────────────┘         └────────┬────────────┘
#               └────────────┬──────┬───────────┘
#                            ▼      ▼
#                      High-Level Features
...
#                            ▼
#              Dense Layer + Dropout + ArcFace Loss
#                            ▼
#                  Final Class Prediction (Softmax)
#
# Description:
# - Each modality (RDI, RAI) is processed independently.
# - Fusion happens after deep, high-level features are extracted.
# - Hence, this is a late fusion strategy.
# ============================================================================
...
def dffn_style_fusion_preserve_shape(input_tensor):
    """
    Applies DFFN-style feature fusion while preserving input shape.
    Input/Output: (batch, 40, 32, 32, 128)
    """
    # Simulate low, mid, high by slicing channels
    FL = input_tensor[..., :42]     # (None, 40, 32, 32, 42)
    FM = input_tensor[..., 42:85]   # (None, 40, 32, 32, 43)
    FH = input_tensor[..., 85:]     # (None, 40, 32, 32, 43)

    # Project all to 128 channels using 1×1×1 conv
    FL_proj = layers.Conv3D(128, kernel_size=1, padding='same', activation='relu')(FL)
    FM_proj = layers.Conv3D(128, kernel_size=1, padding='same', activation='relu')(FM)
    FH_proj = layers.Conv3D(128, kernel_size=1, padding='same', activation='relu')(FH)

    # Elementwise summation (same shape): (None, 40, 32, 32, 128)
    fused = layers.Add()([FL_proj, FM_proj, FH_proj])

    return fused  # (None, 40, 32, 32, 128)
...
print(f"size with entering the dfnn module {conv23.shape}")
conv23_dffn_out= dffn_style_fusion_preserve_shape(conv23)
...
print("conv23_dfnn",conv23_dffn_out.shape)
conv1_tea1 = flatten_temporal(conv23_dffn_out)  # Flatten

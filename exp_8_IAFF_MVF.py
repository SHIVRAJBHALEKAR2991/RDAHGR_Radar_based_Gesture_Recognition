...
#             │              MVF MODULE            │
#             │ ────────────────────────────────── │
#             │ • Conv3D on T-view (k=3x1x1)       │
#             │ • Conv3D on H-view (k=1x3x1)       │
#             │ • Conv3D on W-view (k=1x1x3)       │
#             │ • ReLU + channel concat (skip)     │
#             └────────────────────────────────────┘
#                            ▼
...
# print("conv23 direct after IAFF ",conv23.shape)
...
#### Output Layer
gap_op = tf.keras.layers.GlobalAveragePooling3D()(mvf_output)
...
np.save('exp_8_IAFF_MVF_history.npy', history.history)

# Save only the architecture
model_json = model.to_json()
with open("exp_8_IAFF_MVF_architecture.json", "w") as json_file:
    json_file.write(model_json)

# Save only the weights
model.save_weights("exp_8_IAFF_MVF_GMN_weights.h5")

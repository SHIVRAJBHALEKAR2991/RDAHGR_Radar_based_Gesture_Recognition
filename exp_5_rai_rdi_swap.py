# ======================== LATE FUSION ARCHITECTURE (SWAPPED INPUTS) ==========================
#
#         ┌────────────────────┐        ┌────────────────────┐
#         │     Input RDI      │        │     Input RAI      │
#         │  (T, H, W, C=4)    │        │  (T, H, W, C=1)    │
...
#                  ▼                             ▼
#      ┌─────────────────────┐         ┌─────────────────────┐
#      │  RAI Feature Branch │         │  RDI Feature Branch │
#      │ ((2+1)D Conv, etc.) │         │  (CT Modules, etc.) │
#      └────────┬────────────┘         └────────┬────────────┘
#               └────────────┬──────┬───────────┘
#                            ▼      ▼
#                      High-Level Features
...
#
# Description:
# - The inputs are swapped for experimental study:
#     - RDI input → RAI feature branch ((2+1)D Convs)
#     - RAI input → RDI feature branch (CT Modules)
# - Fusion still occurs at high-level feature stage.
# - This maintains the late fusion strategy.
# =============================================================================================
...
#### RAI
conv11_rai = two_plus_oneDConv(32, 3, 32, 32, 4, 40)  # <-- previously 1
conv12_rai = two_plus_oneDConv(32, 3, 32, 32, 32 + 4, 40)
conv13_rai = two_plus_oneDConv(32, 3, 32, 32, 32 + 32 + 4, 40)
...
# Feed RAI input to RDI branch (i.e., CT_Module backbone)
conv_up1 = conv_up1(Input_Layer_rai)
...
# Feed RDI input to RAI branch (i.e., two_plus_oneDConv backbone)
conv11_rai = conv11_rai(Input_Layer_rdi)
conv11_rai = tf.keras.layers.Concatenate(axis=-1)([conv11_rai, Input_Layer_rdi])
...
np.save('exp_5_rai_rdi_swap_history.npy', history.history)

# Save only the architecture
model_json = model.to_json()
with open("exp_5_rai_rdi_swap_architecture.json", "w") as json_file:
    json_file.write(model_json)

# Save only the weights
model.save_weights("exp_5_rai_rdi_swap.weights.h5")

...
#                      ▼                   ▼
#           ┌────────────────┐   ┌────────────────┐
#           │ RDI Backbone   │   │ RAI Backbone   │
#           └──────┬─────────┘   └──────┬─────────┘
#                  ▼                   ▼
#       ┌─────────────────────────────────────────┐
#       │       Feature Maps from RDI & RAI       │
#       │           (shape: T, H, W, C)           │
#       └────────────────────┬────────────────────┘
#                            ▼
#              iAFF Fusion of RDI and RAI Features
#                            ▼
#                ┌────────────────────────────────┐
#                │          GMN MODULE            │
...
###### iAFF_3D Attention feature fusion
...
class RAIRDI_FusionBlock(tf.keras.layers.Layer):
    def __init__(self, channels=64):
        super(RAIRDI_FusionBlock, self).__init__()
        self.project_rai = layers.Conv3D(channels, kernel_size=1, padding='same', activation=None)
        self.project_rdi = layers.Conv3D(channels, kernel_size=1, padding='same', activation=None)
        self.bn = layers.BatchNormalization()
        self.relu = layers.ReLU()
        self.fusion = layers.Conv3D(channels, kernel_size=1, padding='same', activation=None)

    def call(self, rai, rdi):
        # Project each modality
        proj_rai = self.project_rai(rai)  # (None, 40, 32, 32, 64)
        proj_rdi = self.project_rdi(rdi)  # (None, 40, 32, 32, 64)

        # Fuse via addition or concatenation + conv
        fused = proj_rai + proj_rdi       # or tf.concat([proj_rai, proj_rdi], axis=-1)
        fused = self.bn(fused)
        fused = self.relu(fused)

        # Optional further transformation
        output = self.fusion(fused)       # (None, 40, 32, 32, 64)
        return output
...
#### i-AFF Layer attention feature fusion

iaff_layer = iAFF_3D(channels=64, reduction=16)
...
print(f"rai shape: {conv23_rai.shape} rdi shape :{conv23_rdi.shape}")

fusion_block = RAIRDI_FusionBlock()
conv23= fusion_block(conv23_rdi, conv23_rai)
#### Attention feature fusion Operation
# conv23 = tf.keras.layers.Concatenate(axis=-1)([Input_Layer_rdi,Input_Layer_rai])
# conv23 = iaff_layer(conv23_rdi, conv23_rai)
# ##### Channel Attention
...
print("conv23 direct after CCA ",conv23.shape)
conv1_tea1 = flatten_temporal(conv23)  # Flatten
...
np.save('exp_6_IAFF_GMN_history.npy', history.history)

# Save only the architecture
model_json = model.to_json()
with open("exp_6_IAFF_GMN_architecture.json", "w") as json_file:
    json_file.write(model_json)

# Save only the weights
model.save_weights("exp_6_IAFF_GMN_weights.h5")

...
class TEA_MTA(tf.keras.layers.Layer):
    ...
    """ TEA Module's Multiple Temporal Aggreagation Block for Capturing Long Range Temporal Dependencies """
    ...
    def __init__(self,N,T,H,W,num_channels):
        ...
        self.H = H # Height of a Frame
        self.W = W # Width of a Frame 
        split_factor = self.num_channels//4 # Factor determining number of channels in each split

        #### Defining Layers
        ### Second Split Convolutions
    ...
    def call(self,X):
        ...
        W = (X.shape[3]) # Width of the Frame
        C = self.num_channels # Number of Channels in the Input
        split_factor = int(self.num_channels//4)

        #### Channel Wise Spllting
        Xi_0 = X[:,:,:,:,0*split_factor:1*split_factor]
        Xi_1 = X[:,:,:,:,1*split_factor:2*split_factor]
        Xi_2 = X[:,:,:,:,2*split_factor:3*split_factor]
        Xi_3 = X[:,:,:,:,3*split_factor:]
        ...
        Xi_1_reshaped_temp = tf.keras.layers.Reshape((self.H*self.W,self.T,split_factor))(Xi_1)   # FIXED
        Xi_1_temp = self.conv_temp_1(Xi_1_reshaped_temp)
        Xi_1_reshaped_spa = tf.keras.layers.Reshape((self.T,self.H,self.W,split_factor))(Xi_1_temp) # FIXED
        Xo_1 = self.conv_spa_1(Xi_1_reshaped_spa) 

        ### 3rd Split
        Xi_2 = tf.keras.layers.Add()([Xo_1,Xi_2])
        Xi_2_reshaped_temp = tf.keras.layers.Reshape((self.H*self.W,self.T,split_factor))(Xi_2)     # FIXED
        Xi_2_temp = self.conv_temp_2(Xi_2_reshaped_temp)
        Xi_2_reshaped_spa = tf.keras.layers.Reshape((self.T,self.H,self.W,split_factor))(Xi_2_temp)    # FIXED
        Xo_2 = self.conv_spa_2(Xi_2_reshaped_spa) 

        ### 4th Split
        Xi_3 = tf.keras.layers.Add()([Xo_2,Xi_3])
        Xi_3_reshaped_temp = tf.keras.layers.Reshape((self.H*self.W,self.T,split_factor))(Xi_3)  # FIXED
        Xi_3_temp = self.conv_temp_3(Xi_3_reshaped_temp)
        Xi_3_reshaped_spa = tf.keras.layers.Reshape((self.T,self.H,self.W,split_factor))(Xi_3_temp) # FIXED
...
class CT_Module(tf.keras.layers.Layer):
    ...
    def __init__(self,T,H,W,C):
        ...
        ##### Defining Layers
        self.conv_k1 = tf.keras.layers.Conv3D(filters=self.k1,kernel_size=(3,3,3),padding='same',
                                              activation='linear',kernel_regularizer=tf.keras.regularizers.l2(1e-5))
        self.conv_k2 = tf.keras.layers.Conv3D(filters=self.k2,kernel_size=(3,3,3),padding='same',
                                              activation='relu',kernel_regularizer=tf.keras.regularizers.l2(1e-5))
    ...
    def call(self,X0):
        ...
        """        
        X0 = tf.keras.layers.Reshape((self.T,self.H,self.W,self.k1,self.k2))(X0) # Breaking the Input into sub-dimensions
        X0 = tf.keras.layers.Reshape((self.k2,self.T,self.H,self.W,self.k1))(X0) # Reshaping X0 for first sub-convolution
        X1 = self.conv_k1(X0) # First Sub-Convolution
        X1 = tf.keras.layers.Reshape((self.k1,self.T,self.H,self.W,self.k2))(X1) #  Reshaping X1 for second sub-convolution
        X2 = self.conv_k2(X1) # Second Sub-Convolution
        X2 = tf.keras.layers.Reshape((self.T,self.H,self.W,self.C))(X2) # Reshaping X2 to original dimensions of X0
...
class Cross_MSECA_Module(tf.keras.layers.Layer):
    ...
    def call(self,X_in):
        ...
        """
        X_in_reshaped = tf.keras.layers.Reshape((self.T*self.H*self.W,self.C))(X_in) # Reshaping the Input to 2D 
...
TEA_MTA_1 = TEA_MTA(8,40,32,32,128)
conv2_TEA1 = tf.keras.layers.Conv2D(filters=128,kernel_size=(1,1),padding='same',
                       activation='relu',kernel_regularizer=tf.keras.regularizers.l2(1e-5))

#### TEA-2
conv1_TEA2 = tf.keras.layers.Conv2D(filters=256,kernel_size=(1,1),padding='same',
                        activation='relu',kernel_regularizer=tf.keras.regularizers.l2(1e-5))
TEA_ME_2 = TEA_ME(8,256)
TEA_MTA_2 = TEA_MTA(8,40,32,32,256)
conv2_TEA2 = tf.keras.layers.Conv2D(filters=256,kernel_size=(1,1),padding='same',
                       activation='relu',kernel_regularizer=tf.keras.regularizers.l2(1e-5))

#### TEA-3
conv1_TEA3 = tf.keras.layers.Conv2D(filters=512,kernel_size=(1,1),padding='same',
                        activation='relu',kernel_regularizer=tf.keras.regularizers.l2(1e-5))
TEA_ME_3 = TEA_ME(16,512)
TEA_MTA_3 = TEA_MTA(8,40,32,32,512)
conv2_TEA3 = tf.keras.layers.Conv2D(filters=512,kernel_size=(1,1),padding='same',
                       activation='relu',kernel_regularizer=tf.keras.regularizers.l2(1e-5))
...
conv2_tea1 = conv2_TEA1(tea_mta1)
tea1_op = tf.keras.layers.Add()([conv1_tea1,conv2_tea1])

#### TEA-2
conv1_tea2 = conv1_TEA2(tea1_op)
#tea_me2 = TEA_ME_2(conv1_tea2)
tea_mta2 = TEA_MTA_2(conv1_tea2)
conv2_tea2 = conv2_TEA2(tea_mta2)
tea2_op = tf.keras.layers.Add()([conv2_tea2,conv1_tea2])

#### TEA-3
conv1_tea3 = conv1_TEA3(tea2_op)
#tea_me3 = TEA_ME_3(conv1_tea3)
tea_mta3 = TEA_MTA_3(conv1_tea3)
conv2_tea3 = conv2_TEA3(tea_mta3)
tea3_op = tf.keras.layers.Add()([conv2_tea3,conv1_tea3])

##### Output Layer
gap_op = tf.keras.layers.GlobalAveragePooling3D()(tea3_op)
...
##### Defining Callbacks
filepath= "./Models/RDAHGR_5050_SOLI.h5"
checkpoint = tf.keras.callbacks.ModelCheckpoint(filepath,monitor='val_accuracy',save_best_only=True,mode='max')

###### Training the Model
history = model.fit((X_train_rdi,X_train_rai,y_train_onehot),y_train_onehot,epochs=250,batch_size=8,
                    validation_data=((X_dev_rdi,X_dev_rai,y_dev_onehot),y_dev_onehot),validation_batch_size=8,
                   callbacks=checkpoint)

##### Saving Training Metrics
np.save('./Model History/RDAHGR_5050_SOLI.npy',history.history)
